...
namespace lslidar_driver {
    static const size_t packet_size = sizeof(lslidar_ls_driver::msg::LslidarLsPacket().data);
////////////////////////////////////////////////////////////////////////
...
/** @brief Get one lslidar packet. */
    int InputSocket::getPacket(lslidar_ls_driver::msg::LslidarLsPacket::UniquePtr &pkt) {
        struct pollfd fds[1];
        fds[0].fd = sockfd_;
        fds[0].events = POLLIN;
        ...
        do {
            ...
            {
                LS_ERROR << "poll() reports lslidar error" << LS_END;
                return 1;
            }
        ...
        if (nbytes == packet_size && sender_address.sin_addr.s_addr == devip_.s_addr) {
            ...
            if (nbytes == packet_size && sender_address.sin_addr.s_addr != devip_.s_addr) 
                // RCLCPP_WARN_THROTTLE(private_nh_->get_logger(), *private_nh_->get_clock(), 2000,
                // "lidar ip parameter error: %s, please reset lidar ip in launch file", inet_ntoa(sender_address.sin_addr));
...
/** @brief Get one lslidar packet. */
    int InputPCAP::getPacket(lslidar_ls_driver::msg::LslidarLsPacket::UniquePtr &pkt) {
