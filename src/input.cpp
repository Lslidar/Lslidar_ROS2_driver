...
/** @brief Get one lslidar packet. */
    int InputSocket::getPacket(lslidar_ch_driver::msg::LslidarPacket::UniquePtr &pkt) {
        struct pollfd fds[1];
        fds[0].fd = sockfd_;
        fds[0].events = POLLIN;
        ...
        if (nbytes == packet_size_ && sender_address.sin_addr.s_addr == devip_.s_addr) {
            ...
            if (nbytes == packet_size_ && sender_address.sin_addr.s_addr != devip_.s_addr) {
                LS_ERROR << "lidar ip parameter error, please reset lidar ip in launch file." << LS_END;
            }
...
/** @brief Get one lslidar packet. */
    int InputPCAP::getPacket(lslidar_ch_driver::msg::LslidarPacket::UniquePtr &pkt) {
