...
/*        switch (frequency) {
    ...
    int LslidarChDriver::convertCoordinate(struct Firing lidardata) {
        ...
        if (0x00 == vert_angle_select) {
            vert_angle = vertical_angle[0];
        } else if (0x01 == vert_angle_select) {
            vert_angle = vertical_angle[1];
        } else if (0x02 == vert_angle_select) {
            vert_angle = vertical_angle[2];
        } else if (0x03 == vert_angle_select) {
            vert_angle = vertical_angle[3];
        }
        vert_angle = vert_angle * 0.001f * DEG_TO_RAD;
        ...
        new_point.line = lidardata.vertical_line >> 4;
    ...
    bool LslidarChDriver::polling() {
        ...
        for (int point_idx = 0; point_idx < packet_len; point_idx += point_len) {
            if ((packet->data[point_idx] == 0xff) && (packet->data[point_idx + 1] == 0xaa) &&
                (packet->data[point_idx + 2] == 0xbb) && (packet->data[point_idx + 3] == 0xdd)) {
                packetType = true;
                lidarpoint_count++;
                point_cloud_timestamp =
                        packet_timestamp - packet_interval_time + point_idx * packet_interval_time / (packet_len * 1.0);
               // RCLCPP_INFO(get_logger(),"pct--------------- = %.9f",point_cloud_timestamp);
            }
            // Compute the time of the point

            point_time = packet_timestamp - packet_interval_time +
                         point_idx * packet_interval_time / (packet_len * 1.0) - point_cloud_timestamp;


              //  RCLCPP_INFO(get_logger(),"pt = %.9f",point_time);


            if (packet->data[1205] == 0x02) {
                if (1 == lidarpoint_count && packet->data[point_idx] < 255) {
                    if (echo_num == 0) {
                        memset(&lidardata, 0, sizeof(lidardata));
                        lidardata.vertical_line = packet->data[point_idx];
                        lidardata.azimuth =
                                (packet->data[point_idx + 1] * 65536 + packet->data[point_idx + 2] * 256 +
                                 packet->data[point_idx + 3]) *
                                0.001f * DEG_TO_RAD;
                        lidardata.distance =
                                (packet->data[point_idx + 4] * 65536 +
                                 packet->data[point_idx + 5] * 256 +
                                 packet->data[point_idx + 6]) * DISTANCE_RESOLUTION;
                        ...
                        lidardata.azimuth =
                                ...
                                0.001f * DEG_TO_RAD;
                        lidardata.distance =
                                (packet->data[point_idx + 8] * 65536 +
                                 packet->data[point_idx + 9] * 256 +
                                 packet->data[point_idx + 10]) * DISTANCE_RESOLUTION;
            ...
            if (packetType && 2 == lidarpoint_count) {
                //("---------------onesweep--------------------------\n");
                {
                    std::unique_lock<std::mutex> lock(pointcloud_lock);
        ...
        while (rclcpp::ok()) {
            ...
            if (rc == 0) {
                ...
                if (difop_packet->data[0] == 0xa5 && difop_packet->data[1] == 0xff && difop_packet->data[2] == 0x00 &&
                    ...
                    double vert_angle1 = (difop_packet->data[662] * 256 + difop_packet->data[663]);
                    if (vert_angle1 > 32767) {
                        vert_angle1 = vert_angle1 - 65535;
                    }
                    vertical_angle[0] = vert_angle1;

                    double vert_angle2 = (difop_packet->data[664] * 256 + difop_packet->data[665]);
                    if (vert_angle2 > 32767) {
                        vert_angle2 = vert_angle2 - 65535;
                    }
                    vertical_angle[1] = vert_angle2;

                    double vert_angle3 = (difop_packet->data[666] * 256 + difop_packet->data[667]);
                    if (vert_angle3 > 32767) {
                        vert_angle3 = vert_angle3 - 65535;
                    }
                    vertical_angle[2] = vert_angle3;

                    double vert_angle4 = (difop_packet->data[668] * 256 + difop_packet->data[669]);
